...
### 代理终端输出（表头 + 分隔 + 表格行 + 底部双行粘性块）

启动时打印表头与分隔行（并在横幅里说明模型一次）；每记录一次请求，追加一行；TTY 下底部始终是**表头 + 统计栏**的双行粘性块，随每次请求整块刷新：
...
底部双行粘性块（表头 + 当天累计，本地时区，与行同格式）：

```
 时间       输入/输出    token总/缓存    费用总/缓存      状态
2026-08-24  33.41M/133.9k  33.55M/33.30M  ￥2.7825/1.5870
```
