...
```
 时间       输入/输出    总token(费用)    缓存命中(费用)      状态
-------------------------------------------------------------------
03:37:42  111.4k/799  112.2k(￥0.0114)  cH 109.8k(￥0.0055)  s200
03:38:55  112.8k/2.2k  115.0k(￥0.0176)  cH 111.4k(￥0.0056)  s200
```

底部统计栏（当天累计，本地时区）：

```
2026-08-24 | p 33.41M c 133.9k t 33.55M(￥2.7825) cH 31.74M(￥1.5870)
```

| 字段 | 含义 |
| --- | --- |
| `输入/输出` | p/c：prompt tokens / completion tokens（`111.4k/799` = 输入 111.4k、输出 799） |
