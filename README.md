...
启动时打印表头与分隔行；每记录一次请求，追加一行；TTY 下最后一行是**底部统计栏**（随每次请求刷新）：

```
 时间      模型                 p输入      c输出     t总token(费用)      cH缓存命中(费用)      模式 状态
------------------------------------------------------------------------------------------------------
 14:32:05  deepseek-v4-flash       1.2k        567     1.8k(￥0.0038)      1.0k(￥0.0001)       s  200
```

底部统计栏（当天累计，本地时区）：

```
2026-08-24 | p 32.54M c 126.9k t 32.67M(￥2.7007) cH 30.87M(￥1.5436)
```
...
- token 数缩写：`<1000` 原样、`<1M` 一位小数 k、以上两位小数 M；模型名超宽截断。
